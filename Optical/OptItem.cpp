...
bool	    OptItem::remChild(IOptItem* child)
{
    int idx = di_children.IndexOf(child, 0);
    if (idx == -1)
	return false;
    di_children >> child;
    child->release();
...
const IOptItem* OptItem::acquire() const
{
    this->di_sync.Acquire();
    ++this->di_opencnt;
    this->di_sync.Release();
    return this;
}

void           OptItem::release() const
{
    this->di_sync.Acquire();
    ASSERT(di_opencnt != 0);
    --this->di_opencnt;
...
{
    if (getFlags() & DIF_Common_RelSize)
    {
