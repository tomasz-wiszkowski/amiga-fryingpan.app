...
bool Disc_CD_R::Init(void)
{
    /*
     * let cdrom initialize its stuff
     */
    if (false == Disc_CD_ROM::Init())
	return false;
...
DriveStatus& Disc_CD_R::CloseDisc(DRT_Close type, int lTrackNo)
{
    if (bWriteSAO)
    {
...
DriveStatus& Disc_CD_R::OnChangeWriteMethod()
{
...
DriveStatus& Disc_CD_R::CheckItemData(const IOptItem *pDI)
{
	...
	if (pDI->getFlags() & DIF_Disc_MasterizeCD)
	{
		...
		return result.Complete(ODE_DiscNotEmpty);
	    }
	...
	if (pDI->getChildCount() == 0)
	{
	    ...
	    pIdx->setFlags(DIF_Common_RelSize);
...
DriveStatus& Disc_CD_R::BeginTrackWrite(const IOptItem*pDI)
{
    Page<Page_Write> &pW = drive.GetWritePage();
	...
	if (wantCDText())
	{
	    ...
	    Notify(result(DRT_Operation_Write_CDText));
...
#warning are we leaking memory here if no cdtext is wanted????
	...
	Notify(result(DRT_Operation_Write_Allocate));
	return result.Complete();
    }
...
DriveStatus& Disc_CD_R::UploadLayout(const IOptItem *pDI)
{
    int8 m, s, f;
    uint32 lBlock;
...
#warning nie mam pewnosci czy CD-MRW czasem tu nie wlezie...
	...
	if (pItem->getChildCount() > 1)
	    return result.Complete(ODE_TooManySessions);
	...
	Notify(result(DRT_Operation_Write_Allocate));
	{
	    cmd_SendCueSheet pSCS(dio, result);
