...
unsigned long MUITree::addEntry(unsigned long parent, void* entry, bool opened)
{
   ...
   return DoMtd(list, ARRAY(MUIM_NListtree_Insert, (int)"*", (int)entry, parent, (unsigned)MUIV_NListtree_Insert_PrevNode_Tail, TNF_LIST | (opened ? TNF_OPEN : 0)));
}
...
VectorT<void*> &MUITree::getSelectedObjects()
{
   ...
   while (true)
   {
      DoMtd(list, ARRAY(MUIM_NListtree_NextSelected, (long)&sel));
