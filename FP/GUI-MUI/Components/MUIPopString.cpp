...
void MUIPopString::addEntry(const void* entry)
{
   if (all == 0)
      return;
   DoMtd(list, ARRAY(MUIM_NList_InsertSingle, (long)entry, (unsigned)MUIV_NList_Insert_Bottom));
}
...
const void *MUIPopString::getSelectedEntry()
{
   ...
   DoMtd(list, ARRAY(MUIM_NList_GetEntry, (unsigned)MUIV_NList_GetEntry_Active, (long)&item));
...
{
   MUIPopup::getObject();
   DoMtd(list, ARRAY(MUIM_Notify, MUIA_NList_DoubleClick, MUIV_EveryTime, (long)popup, 2, MUIM_Popstring_Close, true));
